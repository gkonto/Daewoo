...
Scanner::Scanner(std::istream &in) : reader_(in)
{
    startScanner();
    addKeywords();
}
...
char Scanner::getOS_IndependentChar()
{
    ...
    if (ch_ == CR || ch_ == LF)
    {
        if (ch_ == CR)
        {
            ...
            {
                error_.setError("expecting line feed character",
                                lineNumber_,
                                columnNumber_);
                return ch_;
            }
...
void Scanner::getNumber()
{
    ...
    if (ch_ != '.')
    {
        ...
        {
            singleDigit = ch_ - '0';
            if (tokenRecord_.tInt() <=
                (std::numeric_limits<int>::max() - singleDigit) / 10)
            {
            ...
            {
                error_.setError(
                    "integer overflow, constant value too large to read",
                    lineNumber_,
                    columnNumber_);
                return;
            }
    ...
    if (ch_ == 'e' || ch_ == 'E')
    {
        ...
        while (true)
        {
            ...
            {
                error_.setError(
                    "exponent overflow maximum value for exponent is " +
        ...
        {
            tokenRecord_.setFloat(tokenRecord_.tFloat() *
                                  std::pow(10.0, evalue));
        }
    }
}
...
void Scanner::getSpecial()
{
    ...
    case '!':
    {
        ...
        {
            error_.setError(
                "unexpecting '=' character after explanation point: " +
                    std::to_string(ch_),
                lineNumber_,
                columnNumber_);
        }
...
Lexer::Lexer(const std::string &input) : input_(input)
{
    if (input_.size() > std::numeric_limits<BufferSize>::max())
    {
        throw std::out_of_range(
            "Input size exceeds maximum allowed BufferSize");
    }
