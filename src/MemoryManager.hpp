...
public:
    TRhodusObject()
        : type_(TBlockType::btGarbage) {}
    virtual ~TRhodusObject() = default;
    void setType(TBlockType type) { type_ = type; }
    bool isGarbage() const { return type_ == TBlockType::btGarbage; }  // garbage nodes should be deleted..
    bool isBound() const { return type_ == TBlockType::btBound; }
    bool isOwned() const { return type_ == TBlockType::btOwned; }
    bool isConstant() const { return type_ == TBlockType::btConstant; }
    int getRhodusObjectSize() const { return sizeof(TBlockType); }
...
public:
    ...
    int getMemoryListSize() const { return sizeOfList; }
