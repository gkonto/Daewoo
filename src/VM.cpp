...
void VM::store(int symTableIndex)
{
    ...
    case TStackRecordType::stInteger:
        module_->symboltable().storeSymbolToTable(symTableIndex,
                                                  record.ivalue());
        break;
...
void VM::multOp()
{
    ...
    else
    {
        throw std::runtime_error(
            "Internal Error: Unsupported datatype in multiply");
    }
}
...
void VM::divOp()
{
    ...
    {
        throw std::runtime_error(
            "Internal Error: Unsupported datatype in dividing");
    }
}
...
void VM::powerOp()
{
    ...
    {
        throw std::runtime_error(
            "Internal Error: Data type not supported by power operator");
    }
}
...
void VM::andOp()
{
    auto &st1 = stack_.pop();
    auto &st2 = stack_.pop();
    if (st1.type() == TStackRecordType::stBoolean &&
        st2.type() == TStackRecordType::stBoolean)
    {
...
void VM::isLt()
{
    ...
    if (st1_type == TStackRecordType::stInteger)
    {
        ...
        {
            throw std::runtime_error(
                "Incompatible types in ''Is Less Than'' Operation");
        }
    }
...
void VM::isGt()
{
    ...
    if (st1_type == TStackRecordType::stInteger)
    {
        ...
        {
            throw std::runtime_error(
                "Incompatible types in ''Is Greater Than'' Operation");
        }
    }
    else if (st1_type == TStackRecordType::stDouble)
    {
...
void VM::isGte()
{
    ...
    if (st1_type == TStackRecordType::stInteger)
    {
        ...
        {
            throw std::runtime_error(
                "Incompatible types in ''Is Greater-Equal Than'' Operation");
        }
    }
    else if (st1_type == TStackRecordType::stDouble)
    {
...
void VM::isEq()
{
    ...
    if (st1_type == TStackRecordType::stBoolean &&
        st2_type == TStackRecordType::stBoolean)
    {
        stack_.push(st1.bvalue() == st2.bvalue());
        return;
    }
    ...
    else if (st1_type == TStackRecordType::stList)
    {
        // TODO
        throw std::runtime_error(
            "VM::isEq> Incompatible types in equality test");
    }
    else
    {
        throw std::runtime_error(
...
void VM::storeLocalSymbol(int index)
{
    ...
    if (record.type() == TStackRecordType::stString &&
        record.svalue() != nullptr)
    {
