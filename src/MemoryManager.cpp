...
TMemoryNode *TMemoryList::addNode(TRhodusObject *node) {
    ...
    if (head == nullptr) {
        head = cursor;  // FIXME what happens if i delete a single node TMemoryList ? possibly crash...
        tail = cursor;
    } else {
        tail->next = cursor;
        tail = cursor;
...
void TMemoryList::freeFirstNode(TMemoryNode *cursor) {
    if (!head) return;  // Ensure head is not null before proceeding
...
void TMemoryList::freeLastNode(TMemoryNode *cursor, TMemoryNode *previous) {
    if (!cursor || !previous) return;  // Ensure cursor and previous are not null

    tail = previous;           // Set tail to previous node
    previous->next = nullptr;  // Ensure previous node does not point to deleted memory
...
void TMemoryList::freeMiddleNode(TMemoryNode *cursor, TMemoryNode *previous) {
    if (!cursor || !previous) return;  // Ensure valid pointers
...
void TMemoryList::freeGarbage() {
    if (!head) return;  // If the list is empty, exit

    TMemoryNode *cursor = head;
    TMemoryNode *previous = nullptr;

    while (cursor) {
        if (cursor->robj->isGarbage()) {  // Check if the node should be deleted
            --sizeOfList;

            if (cursor == head) {  // If it's the first node
                freeFirstNode(cursor);
            } else {
                if (!cursor->next) {  // If it's the last node
...
TMemorySummary TMemoryList::getNumberAllocatedNodes() {
    ...
    while (temp) {
        ...
        if (temp->robj->isConstant()) ++result.numConstant;
        if (temp->robj->isGarbage()) ++result.numGarbage;
        if (temp->robj->isBound()) ++result.numBound;
        if (temp->robj->isOwned()) ++result.numOwned;
...
std::string TMemoryList::mapMemory() {
    ...
    while (cursor) {
        if (cursor->robj->isBound()) result += 'B';
        if (cursor->robj->isOwned()) result += 'O';
        if (cursor->robj->isGarbage()) result += 'G';
