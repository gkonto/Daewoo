...
TListObject *TListObject::multiply(int multiplier, const TListObject *aList) {
    ...
    if (aList->isBound()) delete workingCopy;  // Clean up cloned copy if needed
...
bool TListObject::listEquals(const TListObject *l1, const TListObject *l2) {
    ...
    for (size_t i = 0; i < l1->list_.size(); ++i) {
        if (!TListItem::listEquals(l1->list_[i], l2->list_[i])) return false;
    }
    return true;
}
...
bool TListItem::listEquals(const TListItem &item1, const TListItem &item2) {
    if (item1.type() == TListItemType::liInteger && item2.type() == TListItemType::liInteger) {
        return item1.ivalue() == item2.ivalue();
    }

    if (item1.type() == TListItemType::liBoolean && item2.type() == TListItemType::liBoolean) {
        return item1.bvalue() == item2.bvalue();
    }

    if (item1.type() == TListItemType::liDouble && item2.type() == TListItemType::liDouble) {
        return item1.dvalue() == item2.dvalue();
    }

    if (item1.type() == TListItemType::liString && item2.type() == TListItemType::liString) {
        return item1.svalue() == item2.svalue();
    }

    if (item1.type() == TListItemType::liList && item2.type() == TListItemType::liList) {
