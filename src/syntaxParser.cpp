...
// that point in time.
std::optional<SyntaxError>
SyntaxParser::expect(TokenCode tc)
{
    if (tokenVector_.token().code() != tc)
    {
        if (error_.has_value())
        {
            return SyntaxError("expecting token: '" +
                               tokenToString(tc) +
                               "' got: '" +
...
std::optional<SyntaxError>
SyntaxParser::expressionStatement()
{
    auto err = expression();
    if (err)
        return err;
...
std::optional<SyntaxError>
SyntaxParser::relationalOperators()
{
    ...
    while (true)
    {
        ...
        if (tc == TokenCode::tLessThan ||
            tc == TokenCode::tLessThanOrEqual ||
            tc == TokenCode::tMoreThan ||
            tc == TokenCode::tMoreThanOrEqual ||
            tc == TokenCode::tNotEqual ||
            tc == TokenCode::tEquivalence)
        {
            nextToken();
...
std::optional<SyntaxError>
SyntaxParser::parseUserDefinedFunction()
{
    std::optional<SyntaxError> err;
...
std::optional<SyntaxError>
SyntaxParser::functionArgumentList()
{
    if (tokenVector_.token().code() == TokenCode::tIdentifier)
    {
...
std::optional<SyntaxError>
SyntaxParser::parseFunctionCall()
{
